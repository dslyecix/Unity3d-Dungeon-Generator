...
public class DungeonGenerator : Singleton {
    ...
    public Transform dungeonObjectParent;
    ...
    private void CreateSpawnRoom()
    {
        ...
        room.transform.parent = dungeonObjectParent.transform;
        ...
        roomsCount++;
        if (replaceBiomes) LoadAppropriateFeatures(room.GetComponent<Room>());
        ...
        for (int i = 0; i < rooms.Count; i++)
        {
            for (int j = 0; j < rooms[i].doors.Count; j++)
            {
                if (rooms[i].doors[j].door == null)
                {
                    ...
                    d.gameObject.transform.parent = dungeonObjectParent.transform;
                }
            }
        }
    }
    ...
    private void GenerateNextRoom() {
        ...
        Debug.Log("Open set length = " + openSet.Count);
        ...
        do {
            ...
            if (roomsCount >= targetRooms) {
                Debug.Log ("We've reached the target number of rooms, start capping them off with dead ends");
                possibleRooms = GetAllRoomsWithOneDoor(possibleRooms);
                Debug.Log("Possible rooms list is now " + possibleRooms.Count + " long");
            }
            ...
            Debug.Log("Rolled a " + r + "for the random room, going to try " + possibleRooms[r].name);
            roomToTry = possibleRooms[r].gameObject;
            doors = roomToTry.GetComponent<Room>().doors.Count;
            Debug.Log("Which has " + doors + " doors");

            //If the room we're adding is a dead end (1 door) and we have multiple possible rooms to pick from
            ...
            newRoom.transform.parent = dungeonObjectParent.transform;
        ...
        if (!roomIsGood) {
            ...
            roomsCount++;
            if (replaceBiomes) LoadAppropriateFeatures(newRoom.GetComponent<Room>());
    ...
    public List<Room> GetAllRoomsWithOneDoor(List<Room> list) {
        ...
        for (int i = 0; i < list.Count; i++) {
            ...
            if (list[i].doors.Count == 1) {
                ...
                Debug.Log("Room " + i + "(" + list[i].name + ") has one door");
            }
        }
